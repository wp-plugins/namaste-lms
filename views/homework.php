...
<form method="post" onsubmit="return namasteValidateForm(this);">
	<div class="postbox namaste-form namaste-box">
		...
		<div><label><?php _e('Description/Requirements:', 'namaste')?></label>
		<?php echo wp_editor(stripslashes(@$homework->description), 'description');?></div>
		<div><input type="checkbox" name="accept_files" value="1" <?php if(!empty($homework->accept_files)) echo 'checked'?>> <?php _e('Accept file upload as solution', 'namaste')?></div>
